...
def run_quadstick(s):
    global oben, unten, rechts, links, feuern, oben_a, unten_a,rechts_a,links_a,feuern_a
    def connect_device():
        ...
        if x.type == 1:
            if x.code == 304:
                ...
                if x.value == 1:
                    print("FEUER")
                    feuern = True
                    s.put("(79,HIGH)") #Feuer low
                if x.value == 0:
                    feuern = False
                    print("FEUER LOSGELASSEN")
                    s.put("(79,LOW)") #Feuer low 

#------------------------- QUAD STICK ENDE -------------------------
                    
#--------------------------- Settings Window ---------------------------
def settings_window(s):
    global oben, unten, rechts, links, feuern, oben_a, unten_a,rechts_a,links_a,feuern_a
    filedirectory = os.path.dirname(os.path.abspath(__file__))
    
    
    settings = tk.Tk()
    settings.title("Configure VK64")
    ...
    # Zeile 1
    lbl_joystick_arrow_up = tk.Label(settings, image=images_dict["pic_joystick_arrow_up"])
    lbl_joystick_arrow_up.place(x=45, y=45, height=40, width=40)

    lbl_joystick_arrow_right = tk.Label(settings, image=images_dict["pic_joystick_arrow_right"])
    lbl_joystick_arrow_right.place(x=974, y=87, height=40, width=40)

    lbl_joystick_arrow_down = tk.Label(settings, image=images_dict["pic_joystick_arrow_down"])
    lbl_joystick_arrow_down.place(x=974, y=129, height=40, width=40)

    lbl_joystick_arrow_left = tk.Label(settings, image=images_dict["pic_joystick_arrow_left"])
    lbl_joystick_arrow_left.place(x=974, y=171, height=40, width=40)

    lbl_joystick_fire = tk.Label(settings, image=images_dict["pic_joystick_fire"])
    ...
    def update_buttons():
        ...
        if(feuern and not feuern_a):
            ...
            feuern_a = False
        settings.after(10, update_buttons)   
...
def run_keyboard(filedirectory, s):
    ...
    def shift_lock(event):
        ...
        if btn_shift_lock.locked: s.put("(51,HIGH)")
        else: s.put("(51,LOW)")
    ...
    def shift(event):
        if event.widget.name == "shift":
            ...
            btn_shift.lock_unlock()
        if btn_shift.locked: s.put("(51,HIGH)")
        else: s.put("(51,LOW)")   
    
    def commodore(event):
        btn_commodore.locked = not btn_commodore.locked
        if btn_commodore.locked: s.put("(50,HIGH)")
        else: s.put("(50,LOW)")
...
def main():
    ...
    settings_thread = threading.Thread(group=None, target=settings_window,args=(data_queue,),daemon=False)

...
def run_quadstick(s):
    global oben, unten, rechts, links, feuern, oben_a, unten_a,rechts_a,links_a,feuern_a, test, gamepad
    ...
    try:
        for event in gamepad.read_loop(): 
            ...
            if(not oben and unten and not rechts and links and letzteRichtung != 6):
                letzteRichtung = 6
                print("unten links")
                s.put(chr(int(176)).encode('latin_1'))
                
            if(not oben and not unten and not rechts and links and letzteRichtung != 7):
                letzteRichtung = 7
                print("links")
                s.put(chr(int(177)).encode('latin_1'))
                
            if(oben and not unten and not rechts and links and letzteRichtung != 8):
                letzteRichtung = 8
                print("oben links")
                s.put(chr(int(178)).encode('latin_1'))
                
            if(not oben and not unten and not rechts and not links and letzteRichtung != 0):
                letzteRichtung = 0
                print("keine")
                s.put(chr(int(179)).encode('latin_1'))

            if x.type == 1:
                """Quadstick Test: x.code == 304:"""
                """Quadstick Tobias: x.code == 306:"""
                if x.code == 304:
                    print(x.value)
                    if x.value == 1:
                        print("FEUER")
                        feuern = True
                        s.put(chr(int(180)).encode('latin_1'))
                    if x.value == 0:
                        feuern = False
                        print("FEUER LOSGELASSEN")
                        s.put(chr(int(80)).encode('latin_1'))
    except:
        gamepad = None
        connect_gamepad()
...
def run_keepalive(s):
    while True:
        s.put(chr(int(254)).encode('latin_1'))
        sleep(10)
        
def find_correct_serial_port():
    while True:
        sleep(2)
        print("Suche Serielle Verindung zu C64!")
        ...
        for port in ports:
            try:
                ...
                arduino.write(chr(int(254)).encode('latin_1'))
                
                response = arduino.readline().decode('latin_1').strip()
                print(f"Antwort von {port}: {response}")
                
                if response == "Ready to go":
                    print("Serielle Verindung zu C64 Hergestellt!")
                    return arduino
...
def main():
    ...
    keepalive_thread = threading.Thread(group=None, target=run_keepalive,args=(data_queue,),daemon=False)
    keepalive_thread.start()
    
    #------------ BLUETOOTH CONNECT ------------
    connected = False
    arduino = find_correct_serial_port()
    
    #------------ BLUETOOTH QUEUE SEND ------------
    while True:
        try:
            if not data_queue.empty():
                arduino.write(data_queue.get())
        except:
            print("Fehler ist Aufgetreten, suche neue Verbindung!")
            arduino = find_correct_serial_port()
